...
@section('table')
    <table id="myTable" class="datatable table-white-space table table-hover card-table zero-configuration table-middle">
        ...
        @forelse ($dataList as $object)
            <tr>
                <td>{{ $object->id }}</td>
                <td>{{ $object->codigo_auto_gen }}</td>
                <td>{{ date("d/m/Y",strtotime($object->created_at)) }}</td>
                <td>
                    <a href="#">{{ $object->driver->nombres }} {{ $object->driver->apellidos }}</a>
                </td>
                <td>
                    <a href="#">{{ $object->cliente->user->name }}</a>
                </td>
                <td>S/ {{ number_format($object->price,2) }}</td>
