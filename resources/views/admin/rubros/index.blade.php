...
<table id="myTable" class="datatable table-white-space table table-hover card-table zero-configuration table-middle">
    ...
    <tbody>
        ...
        <tr>
            <td>{{ $object->rubro_id }}</td>
            <td><a data-modal="byUrl" href="{{ route(str_slug($entity_p).".edit",[$object->rubro_id]) }}">{{ $object->rubro_nombre }}</a></td>
            <td><span class="tag tag-default">{{ $object->rubro_url }}</span></td>
            <td class="text-center has-action">
                <span class="dropdown">
                    <form id="form-delete-{{ $object->rubro_id }}" action="{{ route( str_slug($entity_p).".destroy",[$object->rubro_id]) }}" method="POST">
                        {{ csrf_field() }}
                            {{ method_field("DELETE") }}
                    </form>
                    ...
                    <span aria-labelledby="btnSearchDrop4" class="dropdown-menu mt-1 dropdown-menu-right">
                        <a data-modal="byUrl" href="{{ route(str_slug($entity_p).".edit",[$object->rubro_id]) }}" class="dropdown-item"><i class="icon-pen3"></i> Editar {{$entity_s}}</a>
                        <a href="javascript:eliminar({{ $object->rubro_id }});" class="dropdown-item"><i class="icon-trash4"></i> Eliminar {{$entity_s}}</a>
                    </span>
                </span>
