...
function infoError(message = '') {
    ...
    iziToast.error({
        title: 'Error',
        message:  message == '' ? 'No se pudo completar la operación' : message,
    });
}
 ...
 * @return {void}
 */
function infoOk(message = '',callback=null) {
    //notie.alert("success",message == '' ? 'La operación se realizo con éxito' : message, 3);
    iziToast.success({
        title: 'OK',
        message: message == '' ? 'La operación se realizo con éxito' : message,
        onClose: callback
    });
}
